#!/usr/bin/env python3
"""
Select the best Pb-centered Cys motif and extract radius-based motifs.

This version uses a literature-grounded Pb-S3 scoring model for PbrR691-like
sites instead of the older arbitrary angle-centered score.

Pipeline:
1. Load a PDB/mmCIF structure.
2. Analyze every Pb atom independently.
3. For each Pb:
   - find nearby Cys SG donors across all chains
   - evaluate all possible Cys3 triads, not only the three nearest Cys atoms
   - compute Pb-S distances
   - compute S-Pb-S angles for diagnostics
   - estimate hemidirected ligand clustering using Pb-to-S unit vectors
   - detect nearby missing residues or observed numbering gaps
   - penalize extra side-chain donor atoms near Pb
4. Discard Pb sites near missing/gap regions if requested.
5. Select the best Pb site.
6. Extract motifs around the selected Pb at several radii.
...
Example:
    python scripts/1a_extract_motif_by_radius_literature_score.py \
        --pdb data/raw/5gpe.cif \
        --metal PB \
        --radii 3 4 5 6 8 \
        --outdir work/exp_01/motif_scan
"""
...
# Conservative side-chain donor atoms. Backbone O/N are intentionally not counted
# as extra donors because they can create noisy penalties in tight protein motifs.
...
# Literature-grounded defaults for PbrR691-like Pb-S3 sites.
# EXAFS and crystallographic work place Pb-S distances around 2.6-2.8 A.
...
def residue_label(atoms, index: int) -> str:
    """Return a compact residue label like A113."""
...
def parse_mmcif_missing_residues(path: Path) -> list[dict[str, Any]]:
    ...
    This is intentionally lightweight. If the table is absent, an empty list is
    returned. For critical analyses, validate this output against the PDB entry.
    """
    ...
    while i < len(lines):
        ...
        resname_tag_candidates = [
            "_pdbx_unobs_or_zero_occ_residues.auth_comp_id",
            "_pdbx_unobs_or_zero_occ_residues.label_comp_id",
        ]

        chain_tag = next((tag for tag in chain_tag_candidates if tag in tag_to_idx), None)
        seq_tag = next((tag for tag in seq_tag_candidates if tag in tag_to_idx), None)
        resname_tag = next((tag for tag in resname_tag_candidates if tag in tag_to_idx), None)
        ...
        for row_line in table_lines:
            ...
            seq_raw = parts[tag_to_idx[seq_tag]]
...
def find_observed_numbering_gaps(atoms) -> dict[str, list[dict[str, int]]]:
    ...
    This is not official missing-residue annotation. It is a practical warning
    based on coordinate numbering gaps.
    """
    gaps = {}
...
def residue_near_missing(
    ...
    """Find official missing residues close in sequence to a residue."""
    nearby = []

    for miss in missing_residues:
        if miss["chain"] != chain:
            continue
        if abs(miss["res_id"] - res_id) <= gap_window:
            nearby.append(miss)
...
def residue_near_observed_gap(
    ...
    """Find observed numbering gaps close in sequence to a residue."""
    nearby = []
...
def is_sidechain_donor(resname: str, atom_name: str) -> bool:
    """Return True if an atom is a conservative side-chain donor candidate."""
...
def find_extra_sidechain_donors(
    ...
    """Find side-chain donor atoms near Pb that are not part of the selected triad."""
    strong = []
    weak = []
...
def build_cys_sg_donors(
    ...
    """Build donor records for all Cys SG atoms within the donor cutoff."""
...
def compute_triad_angles(triad: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Compute all pairwise S-Pb-S angles for a Cys3 triad."""
    angles = []
...
def score_pb_s_distances(
    ...
    per_distance = [
        gaussian_score(distance, target=target_pb_s, sigma=sigma_pb_s)
...
def score_hemidirected_vectors(triad: list[dict[str, Any]]) -> float:
    ...
    centroid_norm = float(np.linalg.norm(unit_vectors.mean(axis=0)))
...
def score_chain_topology(triad: list[dict[str, Any]]) -> float:
    ...
    # PbrR691-like metal binding uses two Cys from one subunit and one Cys from
    # the partner subunit. This should be a bonus, not a universal rule.
    if counts == [2, 1]:
        return 1.0
...
def classify_geometry(
    ...
    """Classify the local Pb geometry using conservative rules."""
    if n_triad != 3:
...
def score_candidate_triad(
    ...
    """Score a Pb-centered Cys3 triad using literature-grounded criteria."""
    if len(triad) != 3:
        return {
            "score": -1000.0,
            "distance_score": 0.0,
    ...
    distance_score = score_pb_s_distances(
        distances=distances,
    ...
    donor_exclusion_score = score_extra_donor_exclusion(
        n_strong_extra_donors=n_strong_extra_donors,
        n_weak_extra_donors=n_weak_extra_donors,
    )
...
def select_best_triad_for_pb(
    ...
    if best is None:
        raise RuntimeError("Unexpected failure while selecting Cys3 triad")
...
def analyze_one_pb_site(
    ...
    if distances:
        mean_pb_s = float(np.mean(distances))
        max_pb_s_deviation = float(max(abs(distance - target_pb_s) for distance in distances))
        pb_s_distance_spread = float(max(distances) - min(distances))
    else:
        mean_pb_s = np.nan
        max_pb_s_deviation = np.nan
        pb_s_distance_spread = np.nan

    if angle_values:
        mean_angle = float(np.mean(angle_values))
        angle_spread = float(max(angle_values) - min(angle_values))
    else:
        mean_angle = np.nan
        angle_spread = np.nan
    ...
    # Remove internal atom indices from output-facing donor records.
    output_triad = []
    for donor in triad:
        clean = dict(donor)
...
def extract_radius_motif(atoms, center_coord: np.ndarray, radius: float) -> list[dict[str, Any]]:
    """Extract all protein residues with any atom within radius of the selected Pb."""
    res_names = normalize(atoms.res_name)
...
def write_outputs(
    ...
    all_sites_path = outdir / "all_pb_sites.json"
    selected_path = outdir / "selected_pb_site.json"
    motifs_path = outdir / "motifs_by_radius.json"
    summary_csv = outdir / "pb_site_summary.csv"
    hotspots_path = outdir / "rfd3_hotspots_by_radius.json"

    with all_sites_path.open("w") as handle:
        json.dump(
            {
    ...
    with motifs_path.open("w") as handle:
    ...
    # RFD3-style residue atom hints by radius.
    # This is not automatically the final RFD3 config; it is a helper output.
    hotspots_by_radius = {}
    for radius, motif in motifs_by_radius.items():
        hotspots_by_radius[radius] = {
            row["residue"]: ",".join(row["atoms_within_radius"])
            for row in motif["residues"]
        }

    with hotspots_path.open("w") as handle:
        json.dump(hotspots_by_radius, handle, indent=2)

    print(f"Wrote: {all_sites_path}")
    print(f"Wrote: {selected_path}")
    print(f"Wrote: {motifs_path}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--metal", default="PB")
    ...
    parser.add_argument(
        "--allow-missing-near",
        action="store_true",
        help="Do not discard Pb sites with nearby missing residues or observed numbering gaps.",
    )
    parser.add_argument(
        "--allow-distorted-pb-s",
        action="store_true",
        help="Do not discard sites whose selected triad has a zero Pb-S distance score.",
    )
    parser.add_argument(
        "--allow-strong-extra-donors",
        action="store_true",
        help="Do not discard sites with strong extra side-chain donor atoms near Pb.",
    )
...
def main() -> None:
    ...
    structure_path = Path(args.pdb).resolve()
    ...
    if not args.allow_missing_near:
        eligible_sites = [
            site for site in eligible_sites
            if not site["has_missing_or_gap_near_triad"]
        ]

    eligible_sites = [
        site for site in eligible_sites
        if site["n_triad_donors_used"] == 3
    ]

    if not args.allow_distorted_pb_s:
        eligible_sites = [
            site for site in eligible_sites
            if site["score_components"]["distance_score"] > 0.0
        ]

    if not args.allow_strong_extra_donors:
        eligible_sites = [
            site for site in eligible_sites
            if site["n_strong_extra_sidechain_donors"] == 0
        ]

    if not eligible_sites:
        # Write diagnostic output before failing.
        diagnostic_outdir = outdir / "diagnostics_no_eligible_site"
        ...
        raise RuntimeError(
            "No eligible Pb site found. Inspect diagnostics_no_eligible_site/all_pb_sites.json, "
            "or rerun with --allow-missing-near, --allow-distorted-pb-s, "
            "or --allow-strong-extra-donors if you want a less strict scan."
        )

    selected_site = max(eligible_sites, key=lambda site: site["score"])
    selected_coord = np.array(selected_site["metal_coord"], dtype=float)

    motifs_by_radius = {}
    for radius in sorted(args.radii):
        motif = extract_radius_motif(
            atoms=atoms,
            center_coord=selected_coord,
            radius=radius,
        )
    ...
    print()
    print("Selected Pb site")
    print("----------------")
    print(f"Site: {selected_site['site_id']}")
    print(f"Metal: {selected_site['metal_label']}")
    print(f"Geometry: {selected_site['geometry_call']}")
    print(f"Score: {selected_site['score']}")
    print(f"Score components: {selected_site['score_components']}")
    print(f"Chains involved: {selected_site['chains_involved']}")
    print(f"Missing/gap near triad: {selected_site['has_missing_or_gap_near_triad']}")
    print(f"Strong extra side-chain donors: {selected_site['n_strong_extra_sidechain_donors']}")
    print(f"Weak extra side-chain donors: {selected_site['n_weak_extra_sidechain_donors']}")
    ...
    for donor in selected_site["triad_donors"]:
        print(
            f"{donor['residue']} "
            f"{donor['resname']}:{donor['atom']} "
            f"Pb-S={donor['pb_s_distance']} A "
            f"missing_or_gap_near={donor['near_missing_or_gap']}"
        )

    print()
    print("Angles")
    print("------")
    for angle in selected_site["angles"]:
        print(
            f"{angle['donor_1']}-{args.metal}-{angle['donor_2']}: "
            f"{angle['angle_s_pb_s']} degrees"
        )

    print()
    print("Motifs by radius")
    print("----------------")
    for radius, motif in motifs_by_radius.items():
        residues = " ".join(row["residue"] for row in motif["residues"])
        print(f"{radius} A | n={motif['n_residues']} | {residues}")
